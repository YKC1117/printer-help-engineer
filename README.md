...
**Engineer Edition｜工程維修參考工具。**

此專案整合公開可取得的原廠文件、支援資料、零件資訊、故障排查方法與去識別化實機案例，提供工程師快速查詢。客戶版仍維護於獨立的 `printer-help-customer`，兩邊內容與用途分開。
...
- 大型資料庫分批渲染、完整索引搜尋。
- 工程師工具內可直接查看版本、Bundle ID、資料筆數與自檢狀態。
- GitHub Actions 自動 Bundle、Manifest Audit、語法檢查、資料完整性／深度檢查與 cache busting。

## GitHub Pages 使用方式

此 Repo 規劃以 **GitHub Pages 公開靜態網站**提供跨裝置使用，不需購買主機或網域。

正式網址預期為：

`https://ykc1117.github.io/printer-help-engineer/`

公司 Windows、iPhone、iPad 都直接開同一個網址，因此查詢資料永遠以 GitHub `main` 的最新版為準，不再依賴公司電腦保持開機。

`index.html` 已加入 `noindex / nofollow / noarchive / nosnippet`，降低一般搜尋引擎主動收錄的機會；但 GitHub Pages 本身仍是公開網站，知道網址的人可以直接存取。

### 案件資料與公開網站的邊界

- 客戶名稱、完整序號、工程師姓名、一般案件備註：只存在使用者目前瀏覽器的 `localStorage`，不會自動寫進 GitHub。
- 要加入共用案例庫的資料必須先經去識別化，提交包會排除 `customer / serial / engineer / notes`。
- 共享案例摘要不得包含電話、Email、統編、客戶名稱或完整序號。
- GitHub 上的案例只保留可重用的維修技術內容。
...
9. GitHub Pages 從 `main / (root)` 發布最新版網站。
...
完修案件先存在工程師目前使用裝置的瀏覽器。案件紀錄可匯出 JSON 備份，換電腦或瀏覽器資料遺失時再匯回。
...
提交包會排除客戶名稱、完整序號、工程師姓名與一般備註；工程師把提交包貼給 ChatGPT，由維護流程審核後加入本 Repo。**網頁本身不保存 GitHub Token，也不會自動把未確認案件上傳。**
...
- `internal-case-library.js`：確認後的去識別化共用案例。
...
客戶版維護於 `printer-help-customer`，只提供客戶安全、基本、可自行操作的 Level 1 排除方式。工程師版即使公開作為查詢工具，也不代表應把 Service、量測、主板、拆機、料號或深度工程判斷直接同步到客戶版。
