...
(function(){
  function runKBIntegrityCheck(){
    ...
    list.forEach((x,i)=>{
      ...
      if(!x.brand)warnings.push(`${tag} 缺少 brand`);
      else brandCount[x.brand]=(brandCount[x.brand]||0)+1;

      if(!x.category)warnings.push(`${tag} 缺少 category`);
      else categoryCount[x.category]=(categoryCount[x.category]||0)+1;

      if(!Array.isArray(x.models)||!x.models.length){
        warnings.push(`${tag} 沒有 models`);
      }else{
        for(const m of x.models){
          if(!m||typeof m!=='string')warnings.push(`${tag} 有無效 model 值`);
          else if(knownModels.size && m!=='ALL' && !knownModels.has(m))warnings.push(`${tag} 的機型不在 catalog：${m}`);
        }
      }
      ...
      if(!Array.isArray(x.flow)||!x.flow.length){
        warnings.push(`${tag} 沒有排查流程 flow`);
      }else{
        x.flow.forEach((step,si)=>{
          if(!Array.isArray(step)||step.length<3)errors.push(`${tag} flow 第 ${si+1} 步格式錯誤`);
          else{
            if(typeof step[0]!=='string'||!step[0].trim())errors.push(`${tag} flow 第 ${si+1} 步缺少標題`);
            if(typeof step[1]!=='string'||!step[1].trim())warnings.push(`${tag} flow 第 ${si+1} 步缺少說明`);
            if(!Array.isArray(step[2])||!step[2].length)warnings.push(`${tag} flow 第 ${si+1} 步沒有結果選項`);
          }
        });
      }
      ...
      if(x.title&&seenTopic.has(topicKey))warnings.push(`可能重複主題：${x.title}（${(x.models||[]).join('/')}）`);
      else if(x.title)seenTopic.set(topicKey,pos);
    });
    ...
    if(errors.length){
      ...
      box.textContent=`⚠️ 維修資料庫自檢發現 ${errors.length} 個結構錯誤，請暫停依賴新增資料並查看 Console。`;
